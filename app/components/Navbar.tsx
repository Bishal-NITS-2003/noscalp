...
export default function Navbar() {
  ...
  const handleDisconnect = useCallback((event: ReactMouseEvent<HTMLButtonElement>) => {
  ...
  return (
    ...
    >
      <div className="mx-auto flex max-w-[1180px] items-center justify-between">
              ...
              <AnimatePresence>
                {isWalletDropdownOpen && (
                  <motion.div
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    exit={{ opacity: 0, y: -6 }}
                    className="absolute right-0 mt-2 w-64 rounded-lg bg-white/10 backdrop-blur-md border border-white/20 p-3"
                  >
                    <div className="text-xs text-white/70 mb-2">Connected Wallet</div>
                    <div className="text-sm text-white font-medium mb-1 capitalize">
                      {walletProviderName ?? "Wallet"}
                    </div>
                    <div className="text-xs text-white/80 mb-3 font-mono break-all">
                      {connectedAddress}
                    </div>
                    <button
                      type="button"
        ...
        >
          <span
            className={`h-0.5 w-6 bg-white transition-all ${isMobileMenuOpen ? "translate-y-2 rotate-45" : ""}`}
          />
                ...
                {authUser && connectedAddress ? (
                  <div className="space-y-2">
                    <div className="rounded-lg bg-white/5 p-3">
                      <div className="text-xs text-white/70 mb-1">Connected: {walletProviderName ?? "Wallet"}</div>
                      <div className="text-sm text-white font-mono">{formatAddress(connectedAddress)}</div>
                    </div>
                    <button
                      type="button"
                      onClick={handleDisconnect}
